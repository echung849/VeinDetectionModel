...
def manage_images(image_dir):
    ...
    for image_file in image_files:
        ...
        # Remove channel and batch dimensions before flattening
        image_tensor = image_tensor.flatten()

        # Resize or truncate/pad to exactly 11 elements
        if image_tensor.numel() > 11:
            image_tensor = image_tensor[:11]
        elif image_tensor.numel() < 11:
            # Pad with zeros if needed
            padding = torch.zeros(11 - image_tensor.numel())
            image_tensor = torch.cat([image_tensor, padding])
